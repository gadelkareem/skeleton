...
func (c *Client) CancelSubscription(id string) (*models.Subscription, error) {
    ...
    upcomingInvoice, err := invoice.GetNext(ps)
    if err != nil {
        return nil, err
    }

    var proratedAmount float64
    for _, i := range upcomingInvoice.Lines.Data {
        if i.Type == stripe.InvoiceLineTypeInvoiceItem && i.Amount < 0 {
            proratedAmount = math.Abs(float64(i.Amount))
            break
        }
    }
    ...
    if proratedAmount > 0 {
        ...
        _, err = refund.New(&stripe.RefundParams{
            ...
            Amount: stripe.Int64(int64(proratedAmount)),
        })
        if err != nil {
            return nil, err
        }
